                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div>
                                                ...
                                                <input type="text" name="name_edit"
                                                    value="{{ old('name', $r->name) }}" class="form-control" required
                                            ...
                                            <div>
                                                ...
                                                <select name="guard_name_edit" id="guard_name_select_edit"
                                                    class="form-control" required placeholder="Masukkan guard name disini"
                                                    disabled>
                                                    <option value="">Pilih guard name</option>
                                                    <option @if (old('guard_name_edit', $r->guard_name) == 'web') selected @endif
                                                        value="web">Web</option>
                                                    <option @if (old('guard_name_edit', $r->guard_name) == 'api') selected @endif
                                            ...
                                            <div>
                                                ...
                                                <div class="">
                                                        ...
                                                        <optgroup label="Daftar Permission">
                                                            @foreach ($permission as $p)
                                                                @if ($r->guard_name == $p->guard_name)
                                                                    <option value="{{ $p->name }}"
                                                                        {{ in_array($p->id, $selected_permisison) ? 'selected' : '' }}>
                                                                        {{ $p->name }}</option>
                                                                @endif
                                                            @endforeach
                                                        </optgroup>
                                                    ...
                                                    @if ($permission->count() <= 0)
                                                        <div class="p-1">
                                                            <small class="text-danger">Tidak ada permission yang tersedia
                                                                silahkan create permission terlebih dahulu</small>
                                                        </div>
                                                    @endif
                                                </div>
                                            </div>
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <div class="row">
                                ...
                                <div class="col-sm-6">
                                    <label class="form-label">Guard Name</label>
                                    <input type="text" name="username" value="{{ old('guard_name') }}"
                                        class="form-control" required>
                                    @error('guard_name')
                                        <div class="text-danger text-sm p-1"><i
                                                class="ph-warning-circle"></i>{{ $message }}</div>
            ...
            <form action="/roles" method="post">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <div>
                            <label class="form-label">Guard Name</label>
                            <select name="guard_name" class="form-control" id="guard_name_select" required
                                placeholder="Masukkan guard name disini">
                                <option value="">Pilih guard name</option>
                        ...
                        <div>
                            <label class="col-form-label col-lg-3">Permisison</label>
                            <div class="">
                                <select data-placeholder="Select a State..." id="permission_select" multiple="multiple"
                                    name="permission[]" class="form-control select select-disabled" required
                                    @if ($permission->count() <= 0) disabled @endif>
                                    {{-- <option></option> --}}
                                    <optgroup label="Daftar Permission">
                                        @foreach ($permission as $p)
                                            <option class="{{ $p->guard_name }}"
                                                {{ in_array($p->name, old('permission', [])) ? 'selected' : '' }}
                                ...
                                @if ($permission->count() <= 0)
                                    <div class="p-1">
                                        <small class="text-danger">Tidak ada permission yang tersedia silahkan create
                                            permission terlebih dahulu</small>
                                    </div>
                                @endif
                            </div>
                        </div>
