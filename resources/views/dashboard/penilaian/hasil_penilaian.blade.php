...
@section('breadscrumb')
    @if (Auth::user()->dosen->nidn == $krs->dosen_mk || Auth::user()->dosen->nidn == $krs->dosen_mk_2)
        ...
        <a href="/koordinator/proyeksaya/{{ $krs->id }}"
            class="breadcrumb-item py-2">{{ $krs->kategori->nama_singkat }}</a>
    @endif
        ...
        <div class="card">
            <div class="card-header d-sm-flex align-items-sm-center py-sm-0">
                ...
                <div class="mt-2 mt-sm-0 ms-sm-auto">
                    @if ($pembimbing_kelompok != null)
                        <a class="fw-semibold" href="/kelompok/{{ $pembimbing_kelompok }}"><i
                                class="ph-arrow-circle-left"></i> Kembali </a>
                    @endif
                </div>
            </div>
            <div class="card-body table-responsive">
                <table class="table datatable-hasil-nilai w-100 scrollable-table table-bordered">
                        ...
                        <tr>
                            ...
                            @foreach ($krs->poin_penilaian as $kpp)
                                <th>
                                    @if ($pembimbing_kelompok != null)
                                        <form
                                            action="/krs/{{ $krs->id }}/hasil_penilaian/penilaian/{{ $kpp->id }}"
                                            method="get">
                                            <input type="hidden" name="kelompok" value="{{ $pembimbing_kelompok }}">
                                            <button class="text-primary bg-transparent border-0 fw-semibold p-0"
                                                class="link"
                                                href="/krs/{{ $krs->id }}/hasil_penilaian/penilaian/{{ $kpp->id }}">{{ $kpp->nama_poin }}</button>
                                        </form>
                                    @else
                                        <a class="link"
                                            href="/krs/{{ $krs->id }}/hasil_penilaian/penilaian/{{ $kpp->id }}">{{ $kpp->nama_poin }}</a><br>
                                    @endif
                                    <small class="fw-light">{{ $kpp->bobot }} %</small>
                                </th>
                            @endforeach
                            <th>Nilai akhir</th>
                        </tr>
                    ...
                    <tbody>
                        @php
                            $krs_hasil_penilaian = $krs->kelompok_mahasiswas->where('deleted_at', null);
                            if ($pembimbing_kelompok != null) {
                                $krs_hasil_penilaian = $krs->kelompok_mahasiswas->where('deleted_at', null)->where('kelompok_id', $pembimbing_kelompok);
                            }
                        @endphp
                                ...
                                @foreach ($kk->kelompok_mahasiswas->where('nim', $kk->nim) as $kkm)
                                    ...
                                    @foreach ($krs->poin_penilaian as $kpp)
                                        @php
                                            $total_nilai +=
                                                $kpp
                                                    ->nilai_mahasiswa($kkm->mahasiswa->nim, $kk->id)
