...
@section('breadscrumb')
    ...
    <a href="/krs/{{$krs->id}}/hasil_penilaian" class="breadcrumb-item py-2"> Hasil Penilaian</a>
        ...
        <div class="card">
            <div class="card-header d-sm-flex align-items-sm-center py-sm-0">
                ...
                <div class="mt-2 ms-sm-auto">
                    <div class="d-flex">
                        <a href="/krs/{{$krs->id}}/hasil_penilaian/penilaian/{{$penilaian->id}}/export" class="btn btn-sm btn-success fw-semibold"><i class="ph-microsoft-excel-logo"></i>&nbsp; EXCEL EXPORT</a>
                    </div>
                </div>
            </div>  
                            ...
                            <tr>
                                ...
                                @foreach ($kk->kelompok_mahasiswa->where('nim',$kk->nim) as $kkm)
                                    ...
                                    <td class="fw-semibold">
                                        <a data-bs-toggle="modal" data-bs-target="#kelompok_nilai_{{$kk->id}}" class="text-primary" href="">{{number_format( $penilaian->nilai_mahasiswa($kkm->mahasiswa->nim, $kk->id)->where('approved_status',true)->sum('nilai'), 2, '.', '')}}</a>

                                     {{-- <small class="text-warning"  data-bs-popup="tooltip" title="Apakah anda ingin membatalkan approve penilaian?"><i class="ph-warning-circle"></i></small> --}}
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <div class="text-justify">
                                                        Apakah anda yakin ingin mengunapprove nilai pada kelompok <span class="fw-semibold text-secondary">{{$kk->nama_kelompok}}</span>?
                                                    </div>
                                                    <div>
                                                        <form method="post" action="/kelompok/{{$kk->id}}/penilaian/{{$penilaian->id}}/approved">
                                                            @csrf
                                                            <input type="hidden" name="unapproved" value="cancel">
                                                            <select class="form-control" name="role_group_id" id="" placeholder="Pilih role penilaian..." required>
                                                                    <option value="">Pilih role</option>
                                                                    @foreach ($penilaian->role_group_penilaian as $prp)
                                                                        <option value="{{$prp->role_group->id}}">{{$prp->role_group->nama}}</option>
                                                                    @endforeach
                                                            </select>
                                                    </div>
                                                </div>
