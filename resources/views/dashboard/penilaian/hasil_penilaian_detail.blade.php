...
@section('breadscrumb')
    ...
    @endif
    <a href="/krs/{{ $krs->id }}/hasil_penilaian{{ $pembimbing_kelompok != null ? '?kelompok=' . $pembimbing_kelompok : '' }} "
        class="breadcrumb-item py-2"> Hasil Penilaian</a>
        ...
        <div class="card">
                    ...
                    @if ($pembimbing_kelompok == null)
                        <div class="d-flex">
                            <a href="/krs/{{ $krs->id }}/hasil_penilaian/penilaian/{{ $penilaian->id }}/export"
                                class="btn btn-sm btn-success fw-semibold"><i class="ph-microsoft-excel-logo"></i>&nbsp;
                                EXCEL
                                EXPORT</a>
                        </div>
                    @endif
            ...
            <div class="card-body table-responsive">
                <table class="table  w-100 scrollable-table table-bordered" style="">
                    ...
                    <tbody>
                        @php
                            $krs_hasil_detail_penilaian = $krs->kelompok_mahasiswas->where('deleted_at', null);
                            if ($pembimbing_kelompok != null) {
                                $krs_hasil_detail_penilaian = $krs->kelompok_mahasiswas->where('deleted_at', null)->where('kelompok_id', $pembimbing_kelompok);
                            }
                        @endphp
                            ...
                            <tr>
                                ...
                                @foreach ($kk->kelompok_mahasiswas->where('nim', $kk->nim) as $kkm)
                                    ...
                                    <td class="fw-semibold">
                                        <a data-bs-toggle="modal" data-bs-target="#kelompok_nilai_{{ $kk->id }}"
                                            class="text-primary"
                                            href="">{{ number_format(
                                                $penilaian->nilai_mahasiswa($kkm->mahasiswa->nim, $kk->id)->where('approved_status', true)->sum('nilai'),
                                                2,
                                                '.',
                                                '',
                                            ) }}</a>
                                                ...
                                                <form method="post"
                                                    ...
                                                    <select class="form-control" name="role_group_id" id=""
                                                        ...
                                                        @foreach ($penilaian->role_group_penilaian as $prp)
                                                            <option value="{{ $prp->role_group->id }}">
                                                                {{ $prp->role_group->nama }}
                                                            </option>
                                                        @endforeach
                                                    </select>
