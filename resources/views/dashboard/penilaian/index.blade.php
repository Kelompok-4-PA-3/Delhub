    ...
    <div class="card">
        <div class="card-header d-sm-flex align-items-sm-center py-sm-0">
            ...
            <div class="mt-2 ms-auto">
                @if (Gate::check('is_pembimbing', $kelompok))
                    <form action="/krs/{{$kelompok->krs->id}}/hasil_penilaian" method="get">
                        <input type="hidden" name="kelompok" value="{{$kelompok->id}}">
                        <button type="submit" class="btn btn-sm btn-indigo fw-light fs-sm fw-semibold"> Hasil nilai mahasiswa</button>
                    </form>
                @endif
            </div>
        </div>
        <div class="card-body">
            ...
            <div class="py-5 w-100 text-center">
                <h3 class="text-muted"><i class="ph-warning mb-1"></i> Poin penilaian belum diverifikasi <i
                        class="ph-warning mb-1"></i></h3>
            ...
            <table class="table">
                ...
                <tr>
                    ...
                    {{-- <th>
                        <div class="fw-semibold d-flex">
                            <a @if ($role_dosen->role_group->komponen_penilaian->where('is_verified',false)->count() > 0
                                || $role_dosen->role_group->komponen_penilaian->count() <= 0) @else
                                    href="/kelompok/{{$kelompok->id}}/penilaian/role_kelompok/{{$role_dosen->id}}/{{$role_dosen->role_group->id}}"
                                    @endif>{{$role_dosen->role_group->nama}}</a>
                            @if ($role_dosen->role_group->komponen_penilaian->where('is_verified',false)->count() > 0 ||
                            $role_dosen->role_group->komponen_penilaian->count() <= 0) <a href="#"
                                class="ph-warning-circle px-1 text-warning" data-bs-popup="tooltip"
                                title="Komponen penilaian pada poin ini belum tersedia atau belum terverifikasi"></a>
                                @endif
                        </div>
                        <div>
                    ...
                    <th>
                        ...
                        <div class="fw-semibold d-flex">
                            <a @if($rr->poin_penilaian->komponen_penilaian->where('is_verified',false)->count() <= 0 &&
                                    ...
                                    $rr->poin_penilaian->role_group_penilaian->where('is_verified',false)->count() <= 0)
                            ...
                            || $rr->poin_penilaian->komponen_penilaian->count() <= 0 || $rr->
                                poin_penilaian->role_group_penilaian->where('is_verified',false)->count() > 0)
                                <a href="#" class="ph-warning-circle px-1 text-warning" data-bs-popup="tooltip"
                                    title="Komponen penilaian pada poin ini belum tersedia atau belum terverifikasi"></a>
                                @endif
                        </div>
                        <div>
                    ...
                    {{-- <th>Total
                        <form action="/kelompok/{{$kelompok->id}}/penilaian/role/{{$role_dosen->id}}/approved"
                            class="mt-1" method="post">
                            @csrf
                            @php
                            $all_approved =
                            $kelompok->nilai_mahasiswa->pluck('approved_status')->every(function($status){
                            // return $status != 1;
                            if ($status == 0) {
                            return 1;
                            }elseif($status == 1){
                            ...
                            <button type="button" class="btn btn-warning btn-sm border-0 fw-semibold"
                                data-bs-toggle="modal" data-bs-target="#cancel_approve"><i class="ph-x-circle"></i>
                                &nbsp; Batalkan</button>
                            @elseif((int)$all_approved == 1)
                            <button type="submit"
                                class="btn btn-success btn-sm border-0 fw-semibold {{$btn_approve == 1 ? 'disabled' : ''}}"><i
                                    class="ph-circle-wavy-check"></i> &nbsp;Approve</button>
                            @endif
                        </form>
                    </th> --}}
                </tr>
                ...
                <tr>
                    ...
                    @foreach($role_dosen->role_group->role_group_penilaian as $rr)
                    @if ($kkm->mahasiswa->nilai_mahasiswa($role_dosen->id, $kelompok->id, $rr->poin_penilaian->id) !=
                    NULL)
                    <td>
                        {{$kkm->mahasiswa->nilai_mahasiswa($role_dosen->id, $kelompok->id,
                        $rr->poin_penilaian->id)->detail_nilai_mahasiswa->sum('nilai')}}
                        {{-- {{$role_dosen->id}} --}}
                    </td>
                    @else
                    <td>
                    ...
                    <td>
                        @if ($kkm->mahasiswa->nilai_mahasiswa($role_dosen->id, $kelompok->id, $rrr->poin_penilaian->id)
                        != NULL)
                        @if ($rrr->poin_penilaian->id == $kkm->mahasiswa->nilai_mahasiswa($role_dosen->id,
                        $kelompok->id, $rrr->poin_penilaian->id)->poin_penilaian_id)
                        @php
                        ${"total_nilai".$kkm->nim} += (($kkm->mahasiswa->nilai_mahasiswa($role_dosen->id, $kelompok->id,
                        $rrr->poin_penilaian->id)->nilai) * ($rrr->poin_penilaian->bobot / 100));
                        @endphp
                        {{
                        $kkm->mahasiswa->nilai_mahasiswa($role_dosen->id, $kelompok->id,
                        $rrr->poin_penilaian->id)->nilai
                        / ($rrr->bobot / 100)
                        }}
                        @else
                        0
