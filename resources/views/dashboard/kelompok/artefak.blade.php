                    ...
                    <div class="d-flex">
                        <a @if($myrole->is_verified)
                            ...
                            &nbsp; @if(!$myrole->is_verified) <i class="ph-warning-circle text-warning"
                                style="cursor:pointer;" data-bs-popup="tooltip" title="Role anda belum diverfikasi"></i>
                            @endif</a>
                    </div>
                </div>
                @endif
        ...
        <div class="row w-100 container">
            ...
            <div class="col-lg-4 col-md-6 p-2">
                <div class="submission-card p-2 border border-2 rounded">
                    <div class="d-flex">
                        <div>
                            <a href="" class="fw-semibold text-primary"><i class="ph-folder"></i> {{$s->deskripsi}}</a>
                            @php
                            $artefak_status = 'Not submitted';
                            $artefak = $s->artefak_kelompok->where('kelompoks_id',
                            $kelompok->id)->where('submission_id',$s->id)->first();
                            @endphp
                            @if ($artefak != NULL)
                            @php
                            $artefak_status = 'submitted';
                            @endphp
                            <div class="text-success fs-sm"><small><i class="ph-checks"></i> Submitted</small></div>
                            @else
                ...
                <div class="modal-dialog modal-md">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title"><i class="ph-warning-circle text-info"></i>&nbsp;Submission Artefak
                                {{$s->deskripsi}}</h5>
                        ...
                        <div class="modal-body">
                            <div>
                                <table class="table fs-sm fw-semibold">
                                    ...
                                    @php
                                    if ( $artefak ){
                                    if ( \Carbon\Carbon::parse($s->deadline) > \Carbon\Carbon::parse($artefak ?
                                    $s->artefak_kelompok->where('kelompoks_id',
                                    $kelompok->id)->where('submission_id',$s->id)->first()->created_at :
                                    Carbon\Carbon::now()) ){
                                    $ontime = true;
                                    }else{
                                    $ontime = NULL;
                                    ...
                                    <tr>'
                                        <td>Waktu submit</td>
                                        <td>{{\Carbon\Carbon::parse($artefak ?
                                            $s->artefak_kelompok->where('kelompoks_id',
                                            $kelompok->id)->first()->created_at : '')}}</td>
                                    </tr>
                            ...
                            @if (!$artefak)
                            <form action="/kelompok/{{$kelompok->id}}/artefak/{{$s->id}}" method="post"
                                class="{{$s->id}}" id="form-file" enctype="multipart/form-data">
                                @csrf
                        ...
                        <div class="modal-footer justify-content-between">
                            ...
                            @else
                            <div class="nav-item">
                                <button href="" class="btn btn-danger dropdown-toggle" data-bs-toggle="dropdown">
                                ...
                                <div class="dropdown-menu">
                                    <form
                                        ...
                                        <div class="container">
                                            ...
                                            <button class="badge border-0 bg-success" type="submit"> ya</button>
                                            <button class="badge border-0 bg-secondary" type="button"> batal</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
...
<script>
    ...
    inputElements.forEach((inputElement) => {
        ...
        FilePond.setOptions({
        server:{
            url: 'http://localhost:8000/artefak/file/temporary',
            process: '/store',
            revert: {
                url: '/delete',
                method: 'POST', 
