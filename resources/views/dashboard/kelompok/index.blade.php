...
<div class="row">
	<div class="mb-2">
		<ul class="nav nav-tabs nav-tabs-highlight nav-justified wmin-lg-100 me-lg-3 mb-3 mb-lg-0">
			...
			@if (array_intersect(Auth::user()->dosen->role_kelompok($kelompok->id)->pluck('id')->toArray(),
			$kelompok->role_kelompok->pluck('id')->toArray())) --}}
	...
	<div class="tab-content">
						...
						<tr>
							...
							<td class="text-center">
								<div class="d-inline-flex">
									...
									@role('dosen')
									@if (Auth::user()->dosen->nidn == $kelompok->krs->dosen_mk ||
									Auth::user()->dosen->nidn == $kelompok->krs->dosen_mk_2)
									@foreach (Auth::user()->dosen->role_kelompok->where('kelompok_id',$kelompok->id) as
									$myrole)
									<small>
										<form action="/kelompok/{{$kelompok->id}}/people/delete" method="post">
											@csrf
											<input type="hidden" name="mahasiswa" value="{{$a->nim}}">
											<input type="hidden" name="kelompok" value="{{$a->kelompok_id}}">
											<button class="text-danger bg-transparent border-0" type="submit"><i
		...
		<div class="tab-pane fade show" id="tambah-anggota">
			<div class="card p-3">
				<div class="p-3">
					<form action="/kelompok/{{$kelompok->id}}/people/add" method="post">
						@csrf
						<p>Tambah Anggota Kelompok</p>
						<input type="hidden" name="kelompok" value="{{$kelompok->id}}">
						<div class="py-2">
							<select data-placeholder="Pilih mahasiswa" name="mahasiswa[]" multiple="multiple"
								...
								<optgroup label="Daftar mahasiswa">
									...
									<option value="{{$km->mahasiswa->nim}}">
										<div>
											{{$km->mahasiswa->user->nama}} - {{$km->mahasiswa->nim}} (<small>Design,
												Coding</small>)<br>
										</div>
									</option>
									@endif
